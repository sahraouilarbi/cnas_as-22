...
export const SectionEtablissement = ({ data, isHosp, isSoins, isSejour }) => (
  <div className="relative official-border pt-4 px-0 margin-between-sections">
    <FloatingTitle
      title="CADRE RÉSERVÉ A L'ÉTABLISSEMENT DE SOINS"
      isUnderline={false}
    />
    <div className="overflow-hidden">
      <div className="p-2 space-y-1">
        ...
        <div className="flex gap-10">
          ...
          <DottedField
            label="Du"
            value={formatDate(data.convention_date_du)}
            className="w-40"
          />
        </div>
        ...
        <div className="flex gap-4">
          ...
          <DottedField
            label="Né(e) le :"
            value={formatDate(data.patient_date_naissance)}
            className="w-40"
          />
        </div>
          ...
          <div className="pl-4 space-y-0.5">
            ...
            <div className="flex gap-3 items-center">
              <DottedField
                label="à compter du :"
                value={
                  isHosp || isSejour ? formatDate(data.hosp_date_debut) : ""
                }
                className="w-40"
              />
              <span className="mx-1">pour :</span>
              <CheckBox
                label="Séjour initial"
                checked={
                  (isHosp || isSejour) && data.hosp_type_sejour === "initial"
                }
              />
              ...
              <DottedField
                ...
                className="w-40"
              />
              <span className="inline-block">jours.</span>
            </div>
            <DottedField
              label="Actes prévus :"
        ...
        <div className="mt-1 pt-1">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="w-2 h-2 rounded-full border border-black inline-block flex-shrink-0"></span>
            <span className="font-bold uppercase text-[11px]">SOINS</span>
          </div>
          <div className="pl-4 space-y-0.5">
            <div className="flex gap-4">
              <DottedField
              ...
              <DottedField
                label="Du"
                value={isSoins ? formatDate(data.soins_periode_du) : ""}
                className="w-32"
              />
              <DottedField
                label="Au"
                value={isSoins ? formatDate(data.soins_periode_au) : ""}
                className="w-32"
              />
            </div>
