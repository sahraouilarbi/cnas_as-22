...
const FormulairePage = () => {
  ...
  useEffect(() => {
    if (config && !isEdit) {
      ...
      setValue(
        ...
        config.etablissement_raison_sociale,
      );
  ...
  const mutation = useMutation({
    mutationFn: (data) =>
      isEdit ? dossiersApi.update(id, data) : dossiersApi.create(data),
    onSuccess: (res) => {
      ...
      <div className="min-h-screen flex flex-col items-center overflow-auto">
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 shadow-lg z-50 flex justify-center gap-4 no-print">
          <button
            ...
            className="px-6 py-2 bg-slate-200 text-slate-700 font-medium hover:bg-slate-300 rounded-md transition-colors flex items-center gap-2"
          >
            <ArrowLeft size={18} /> Retour à la saisie
          </button>
          <button
            onClick={() => window.print()}
            className="btn-primary px-10 flex items-center gap-2 shadow-xl"
          >
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-slate-800">
              {isEdit
                ...
                : "Nouvel Engagement de Prise en Charge"}
            </h1>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <section className="section-card relative">
            <div className="flex items-center gap-2 mb-4 border-b border-slate-100 pb-2">
              ...
              <h2 className="text-lg font-semibold text-slate-800">
                1. Informations Assuré Social
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 relative">
                <label className="input-label">
                  N° d'immatriculation (12 chiffres)
                </label>
                <input
                  ...
                  autoComplete="off"
                />
                {errors.assure_num_immatriculation && (
              ...
              <div className="md:col-span-2">
                <label className="input-label">
                  Nom et Prénom(s) de l'assuré(e)
                </label>
                <input
                  {...register("assure_nom_prenom", { required: true })}
                  className="input-field"
                  placeholder="Ex: BENALI Mohamed"
                />
                {errors.assure_nom_prenom && (
                  <span className="text-xs text-red-500">
              ...
              <div>
                ...
                <input
                  type="date"
                  {...register("assure_date_naissance", { required: true })}
                  className="input-field"
                />
              </div>
              <div>
                ...
                <input
                  {...register("assure_lieu_naissance", { required: true })}
                  className="input-field"
                />
              </div>
              <div>
                ...
                <input
                  {...register("assure_agence")}
                  className="input-field"
                  placeholder="Ex: Agence Ain Tedles"
                />
              </div>
              <div>
                <label className="input-label">N° Téléphone</label>
                <input
                  {...register("assure_telephone")}
                  className="input-field"
          ...
          <section className="section-card">
            <div className="flex items-center gap-2 mb-4 border-b border-slate-100 pb-2">
              ...
              <h2 className="text-lg font-semibold text-slate-800">
                2. Établissement de Soins & Patient
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4 border-r border-slate-100 pr-4">
                ...
                <div>
                  <label className="input-label">
                    Établissement de soins (Nom)
                  </label>
                  <input
                    {...register("etablissement_nom")}
                ...
                <div className="p-3 bg-blue-50 rounded-md flex items-start gap-2">
                  ...
                  <p className="text-[11px] text-blue-700">
                    Ces informations sont modifiables dans les paramètres de
                    l'application.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                    ...
                    {["hospitalisation", "soins", "sejour"].map((t) => (
                      ...
                      >
                        <input
                          ...
                          className="text-cnas-blue"
                        />
                        <span className="capitalize">
                          {t === "soins" ? "Soins (Dialyse)" : t}
                        </span>
                      </label>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-y-2 mt-2">
                    {["assure", "conjoint", "enfant", "ascendant", "autre"].map(
                      (lien) => (
                        <label
                          key={lien}
                          className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer"
                        >
                          <input
                            type="radio"
                            value={lien}
                            {...register("patient_lien")}
                            className="text-cnas-blue"
                          />
                          <span className="capitalize">
                            {lien === "assure" ? "Lui-même" : lien}
                          </span>
                        </label>
                      ),
                    )}
                ...
                <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-50">
                  <div className="col-span-2">
                    ...
                    <input
                      {...register("patient_nom", { required: true })}
                      className={`input-field ${patientIsAssure ? "bg-slate-50" : ""}`}
                      readOnly={patientIsAssure}
                    />
                  </div>
                  <div className="col-span-2">
                    ...
                    <input
                      type="date"
                      {...register("patient_date_naissance", {
                        required: true,
                      })}
                      className={`input-field ${patientIsAssure ? "bg-slate-50" : ""}`}
                      readOnly={patientIsAssure}
          ...
          {typeDemande === "soins" ? (
            ...
            <section className="section-card border-l-4 border-l-orange-400 animate-in fade-in slide-in-from-left-2 duration-300">
              <div className="flex items-center gap-2 mb-4 border-b border-slate-100 pb-2">
                ...
                <h2 className="text-lg font-semibold text-slate-800">
                  3. Cadre Hospitalisation / Séjour
                </h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    {...register("hosp_type_chirurgie")}
                    className="input-field"
                  >
                    <option value="chirurgie">Chirurgie</option>
                    <option value="chir_cardio">
                ...
                {watch("hosp_type_chirurgie") === "autre" && (
                  <div>
                    ...
                    <input
                      {...register("hosp_type_chirurgie_autre")}
                      className="input-field"
                    />
                  </div>
                )}
